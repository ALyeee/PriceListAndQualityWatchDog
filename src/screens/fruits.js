...
export default function Fruits() {
  ...
  const GetFruits = () => {
    axios
      .get(
        `http://localhost:8089/myfyp/api/dummy/getRecordByCatagoryAndCity?cat=FRUIT&city=${userData?.city}`,
      )
      .then(function (response) {
        ...
        if (response.data === 'Found') {
          ...
          response?.data?.map(res => {
            arrayFr = [res.pName, res.pUnit, res.pPrice, 1];
            arrayFr1 = [res.pName, res.pUnit, res.pPrice];
            newArr.push(arrayFr);
  ...
  const EditFruit = () => {
    axios
      .post(
        `http://localhost:8089/myfyp/api/dummy/updateFruit?city=${userData?.city}`,
        {
          pName: fruitName,
          pPrice: price,
          pUnit: unit,
  ...
  const DeleteFruit = data => {
    ...
    axios
      .post(
        `http://localhost:8089/myfyp/api/dummy/deletefruit?city=${userData?.city}`,
        {
          pName: data[0],
          pPrice: data[1],
          pUnit: data[2],
  ...
  const OnChangeCity = city => {
    axios
      .get(
        `http://localhost:8089/myfyp/api/dummy/getRecordByCatagoryAndCity?cat=FRUIT&city=${city}`,
      )
      .then(function (response) {
        console.log(response);
        if (response.data === 'Found') {
  ...
  return (
    <View style={{flex: 1}}>
        ...
        <View style={{minWidth: '90%'}}>
          <TouchableOpacity
            ...
            <Text style={{color: '#000', padding: 5}}>
              {userData?.role === 'admin' ? userData?.city : selected}
            </Text>
          </TouchableOpacity>
        </View>
        {userData?.role === 'admin' ? (
                    ...
                    {rowData.map((cellData, cellIndex) => {
                      return (
                        <Cell
                          key={cellIndex}
                          data={
                            cellIndex === 3 ? element(rowData, index) : cellData
                          }
      ...
      <NativeBaseProvider>
        ...
        <Modal isOpen={visible} onClose={() => setVisible(false)}>
          <Modal.Content maxWidth="500px">
            <Modal.CloseButton />
            <Modal.Header>Add Fruit</Modal.Header>
            <Modal.Body>
              <TextInput
                style={{
                  minWidth: '80%',
      ...
      <Snackbar
        ...
        {!isEdit ? 'Fruit Added SuccessFully' : 'Fruit Updated Successfully'}
      </Snackbar>
      <Snackbar
        visible={isDeleted}
