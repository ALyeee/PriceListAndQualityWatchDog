...
export default function HallOfShame({navigation}) {
  ...
  const UserData = async () => {
    try {
      ...
      setSelected(JSON.parse(jsonValue.city));
      OnChangeCity(userCity.city);
    } catch (e) {
      // error reading value
    }
  };
  ...
  const OnChangeCity = async city => {
    ...
    // setSelected(jsonValue.city);
    // console.log(jsonValue.email);
    axios
      .get(
        `http://localhost:8089/myfyp/api/dummy/getHallOfShameByCity?city=${city}`,
      )
  ...
  const Item = ({item, onPress, backgroundColor, textColor}) => (
    <TouchableOpacity onPress={onPress} style={[styles.item, backgroundColor]}>
      <View style={styles.card}>
        <Text style={styles.amount}>{item.ShopName}</Text>
  ...
  return (
    <View>
      <View
        ...
        <Text style={{fontWeight: 'bold', fontSize: 30}}>Hall Of Shame</Text>
      </View>
